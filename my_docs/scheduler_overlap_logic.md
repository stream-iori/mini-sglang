...
| `write_indices` | `Tensor` | 1D indices mapping where to write the generated *next* tokens. |
...
*   **Goal:** Decide what runs *next* on the GPU.
*   **Logic:**
...
*   **Logic:**
    *   If `forward_input` exists:
        ...
        *   Call `self._forward(forward_input)`.
...
*   **Logic:**
    ...
    *   *Note:* `ongoing_data` is passed here mainly to ensure we don't accidentally free resources for requests that are still running in the *current* batch (edge cases).
