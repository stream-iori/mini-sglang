    ...
    def forward(
        ...
        for i, req in enumerate(batch.reqs):
            ...
            page_indices = self.page_table[
                req.table_idx, : (kv_len + self.kvcache.page_size - 1) // self.kvcache.page_size
            ]
            ...
            while cur_written < req.extend_len:
                ...
                self.kvcache.pool[page_id, 0, :, page_offset : page_offset + write_len, :] = cur_k[
                    cur_written : cur_written + write_len
                ].permute(1, 0, 2)
                self.kvcache.pool[page_id, 1, :, page_offset : page_offset + write_len, :] = cur_v[
                    cur_written : cur_written + write_len
                ].permute(1, 0, 2)
            ...
            attn_out = F.scaled_dot_product_attention(
                q_in, k_in, v_in, is_causal=batch.is_prefill, scale=self.scaling
